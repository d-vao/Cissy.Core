...
namespace Cissy.RateLimit
{
    public abstract class RateLimitProcessor
    {
        ...
        public virtual async Task<RateLimitCounter> ProcessRequestAsync(ClientRequestIdentity requestIdentity, RateLimitRule rule)
        {
            ...
            using (await AsyncLock.WriterLockAsync(counterId).ConfigureAwait(false))
            {
                ...
                if (entry.IsNotNull())
                {
                    ...
                    if (entry.Timestamp + rule.PeriodTimespan.Value >= DateTime.UtcNow)
                    {
                        ...
                        var totalCount = entry.Count + _config.RateIncrementer?.Invoke() ?? 1;
                ...
                await _counterStore.SetAsync(counterId, counter, rule.PeriodTimespan.Value);
            }
        ...
        public virtual RateLimitHeaders GetRateLimitHeaders(RateLimitCounter counter, RateLimitRule rule)
        {
            ...
            {
                reset = counter.Timestamp + (rule.PeriodTimespan ?? rule.Period.ToTimeSpan());
