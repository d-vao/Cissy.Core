...
namespace Cissy.Dapper.Sql
{
    public interface ISqlGenerator
    {
        ICissyDapperConfiguration Configuration { get; }
        ...
        string LockUpdate(IClassMapper classMap, IPredicate predicate, IDictionary<string, object> parameters, int DateVersion);
    ...
    {
        public SqlGeneratorImpl(ICissyDapperConfiguration configuration)
        {
            Configuration = configuration;
        }

        public ICissyDapperConfiguration Configuration { get; private set; }
        ...
        public virtual string LockUpdate(IClassMapper classMap, IPredicate predicate, IDictionary<string, object> parameters, int DataVersion)
        {
            if (predicate == null)
            {
                throw new ArgumentNullException("Predicate");
            ...
            var cn = GetColumnName(classMap, columns.FirstOrDefault(m => m.ColumnName == DataVersionMeta.DataVersionColumName), false);
            var sl = $"{cn} = {cn}+1";
            var sc = $"{cn} < {DataVersion + 1}";
            var setSql =
                columns.Where(m => m.ColumnName != DataVersionMeta.DataVersionColumName).Select(
                    p =>
                        string.Format(
                            "{0} = {1}{2}", GetColumnName(classMap, p, false), Configuration.Dialect.ParameterPrefix, p.Name));
            return $"UPDATE {GetTableName(classMap)} SET {sl},{setSql.AppendStrings()} WHERE {predicate.GetSql(this, parameters)} AND {sc}";
        }
