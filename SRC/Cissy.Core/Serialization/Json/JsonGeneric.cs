...
namespace Cissy.Serialization.Json
{
    public class jsonCovert
    {
        ...
        public string GenericToJson(object item)
        {
            StringBuilder Json = new StringBuilder();
            ...
            for (int j = 0; j < pi.Length; j++)
            {
                Type type = pi[j].GetValue(item, null).GetType();
                Json.Append("\"" + pi[j].Name.ToString() + "\":" + StringFormat(pi[j].GetValue(item, null).ToString(), type));
